...
function findMinMax([firstElement, ...restElements]) {
  ...
  let minimal = firstElement;
  let maximal = firstElement;
  for (let i = 0; i < restElements.length; i++) {
    if (restElements[i] < minimal) {
      minimal = restElements[i];
    } else if (restElements[i] > maximal) {
      maximal = restElements[i];
    }
  }
  console.log("Min:", minimal, "Max:", maximal);
}
...
function replaceNumbers(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] > 10) {
      result = arr[i] + "Greater than 10";
    }
    if (arr[i] <= 10) {
      result = arr[i] + "Less than or equal to 10";
    }
    console.log(result);
  }
}
